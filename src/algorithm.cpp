...
pair<pi, int> backtrace(int u, int &obj)
{
    ...
    dfs(revadj, visited, ntype, state, u, obj, assign, backtraced);
    return assign;
}

void dfs(vvp &revadj, vi &visited, vi &ntype, vi &state, int node, int &obj, pair<pi, int> &assign, bool &backtraced)
{
    visited[node - 1] = 1;
    cout << "\n" << node << "\n";
    int v, w;
